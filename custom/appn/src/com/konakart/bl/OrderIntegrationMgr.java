...
public class OrderIntegrationMgr extends BaseMgr implements OrderIntegrationMgrInterface
{
    ...
    public OrderIf beforeSaveOrder(OrderIf order)
    {
        ...
        {
            log.info("Before save of Order (LifecycleId: " + order.getLifecycleId() + " OrderNumber:"
    ...
    public void saveOrder(OrderIf order)
    {
        ...
        if (order.getStatus() == com.konakart.bl.OrderMgr.PAYMENT_RECEIVED_STATUS)
        {
            ...
            exportOrder(order, KKConstants.UPDATE_OPERATION_TYPE_CREATE);
    ...
    public void changeOrderStatus(int orderId, int currentStatus, int newStatus)
    {
        ...
        try
        {
            ...
            if (newStatus == OrderMgr.PAYMENT_RECEIVED_STATUS)
            {
                ...
                manageGiftCertificates(order);
                manageGiftRegistries(order);
                exportOrder(order, KKConstants.UPDATE_OPERATION_TYPE_UPDATE);
                removeStockReservations(order);
    ...
    public void afterInsertSubscription(SubscriptionIf subscription) throws Exception
    {
        ...
        if (false)
        {
            ...
            /*
             * Create an AdminEngineMgr, if possible (need Business or Enterprise Editions for this)
             */
    ...
    public PdfResult createInvoice(OrderIf order)
    {
        ...
        try
        {
            ...
            if (Utils.isBlank(order.getLocale()))
            {
                if (log.isDebugEnabled())
                {
                    log.debug("Locale on OrderId " + order.getId() + " is null or blank");
                }
            } else if (order.getLocale().length() < 2)
            {
                if (log.isDebugEnabled())
                {
                    log.debug("Locale on OrderId " + order.getId() + " is invalid: " + order.getLocale());
                }
            } else
            {
                if (log.isDebugEnabled())
     ...
     * 
     * @param order
     *            The order to post to the message queue
     * @param operationType
     *            The type of operation
     */
    private void exportOrder(OrderIf order, int operationType)
    {
        if (log.isDebugEnabled())
        {
            log.debug("Export order operation type " + operationType);
        }
        ...
        try
        {
            ...
            if (mgr.isERPEnabled())
            {
                if (log.isDebugEnabled())
                {
                    log.debug("ERP is enabled");
                }

                if (mgr.isExportOrder())
                {
                    if (log.isDebugEnabled())
                    {
                        log.debug("call exportOrder");
                    }
                    mgr.exportOrder(getStoreId(), order);
                }

                if (mgr.isUpdateObjectEnabled(KKConstants.UPDATE_OBJECT_TYPE_ORDER, operationType))
                {
                    if (log.isDebugEnabled())
                    {
                        log.debug("call updateObject");
                    }
                    mgr.updateObject(getStoreId(), KKConstants.UPDATE_OBJECT_TYPE_ORDER,
                            operationType, order.getId(), /* objectId1 */null, /* objectId2 */null,
                            /* value1 */Integer.toString(order.getStatus()), /* value2 */null);
                }
            } else
            {
                if (log.isDebugEnabled())
                {
                    log.debug("ERP is not enabled");
                }
            }
        } catch (Exception e)
        {
    ...
    private void manageRewardPoints(OrderIf order)
    {
        ...
        try
        {
            ...
            if (conf != null && conf.getValue() != null && conf.getValue().equalsIgnoreCase("true"))
            {
                if (log.isDebugEnabled())
                {
                    log.debug("Entered manageRewardPoints() - Reward Points Enabled");
                }
                OrderMgrIf orderMgr = getOrderMgr();
            ...
            } else
            {
                if (log.isDebugEnabled())
                {
                    log.debug("Entered manageRewardPoints() - Reward Points Disabled");
                }
            }
        } catch (Exception e)
        {
    ...
    private void manageTax(OrderIf order)
    {
        ...
        {
            TaxModuleData tmd = getTaxModuleMap().get(ot.getClassName());
