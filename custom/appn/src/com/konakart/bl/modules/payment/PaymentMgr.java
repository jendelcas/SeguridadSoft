...
public class PaymentMgr extends BaseMgr implements PaymentMgrIf
{
    ...
    public void refreshConfigs() throws Exception
    {
        synchronized (mutex)
        {
            ...
            if (log.isInfoEnabled())
            {
                ...
                String staticD = "Configuration data for the PaymentMgr on " + getStoreId();
                staticD += "\n\t\t\t Price With Tax?    = " + sd.isDisplayPriceWithTax();
                staticD += "\n\t\t\t Store Name         = " + sd.getStoreName();
                staticD += "\n\t\t\t Module List        = " + sd.getPaymentModuleList().size()
                        + " modules";
            ...
            throws Exception
    {
        ...
        if (paymentDetails != null && paymentDetails.length > 0)
        {
            if (subCode != null)
            {
                ...
                if (log.isDebugEnabled())
                {
                    log.debug("Payment module with code " + moduleCode + " and subCode = " + subCode
                            + " not found");
                }
            }
            ...
            if (modName != null)
            {
                try
                {
                    ...
                    if (paymentModule.isAvailable())
                    {
                        ...
                        PaymentDetails payDet = paymentModule.getPaymentDetails((Order) order,
                                info);
                                ...
                                && payDet.getPaymentDetails().length > 0)
                        {
                            for (int i = 0; i < payDet.getPaymentDetails().length; i++)
                            {
                                ...
                                {
                                    log.debug(
                                            "PaymentModule " + modName + "\n\t SubTypes Available: "
                                                    + " code = " + subPayDet.getCode()
                                                    + " subCode = " + subPayDet.getSubCode()
                                                    + " title = " + subPayDet.getTitle());
                                }
                            }
                        } else
                        {
                ...
                } catch (Exception e)
                {
                    if (moduleName == null)
                    {
                        log.error("Could not instantiate the Payment Module " + modName + " ("
                                + modName + ")", e);
    ...
    protected void addCurrencyToOrder(Order order) throws Exception
    {
        ...
        if (order.getCurrency() == null)
        {
            ...
            {
                throw new KKException(
                        "Cannot find a currency for currency code = " + order.getCurrencyCode());
            }
