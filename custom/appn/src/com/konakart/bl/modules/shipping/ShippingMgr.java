...
public class ShippingMgr extends BaseMgr implements ShippingMgrIf
{
    ...
    public void refreshConfigs() throws Exception
    {
        synchronized (mutex)
        {
            ...
            if (originCountry != KKConstants.NOT_SET)
            {
                ...
                if (sd.getShippingOriginCountry() == null)
                {
                    log.warn("Shipping Origin Country (" + originCountry + ") not found for "
                            + getStoreId());
                }
            }
            ...
            if (log.isInfoEnabled())
            {
                ...
                String staticD = "Configuration data for the ShippingMgr on " + getStoreId();
                if (sd.getShippingOriginCountry() == null)
                {
                    staticD += "\n\t\t\t Origin Country     = null for id " + originCountry;
                } else
                {
                    staticD += "\n\t\t\t Origin Country     = "
                            + sd.getShippingOriginCountry().getName();
                }
                staticD += "\n\t\t\t Origin Zip         = " + sd.getShippingOriginZip();
                staticD += "\n\t\t\t Box Weight         = " + sd.getShippingBoxWeight();
                staticD += "\n\t\t\t Max Weight         = " + sd.getShippingMaxWeight();
                staticD += "\n\t\t\t Free Shipping?     = " + sd.isFreeShipping();
            ...
            int languageId) throws Exception
    {
        ...
        int numProducts = 0, numDigitalDownloads = 0, numBookableProds = 0, numVirtualProds = 0,
                numGiftCertificates = 0, numFreeShipping = 0;
        BigDecimal weight = new BigDecimal(0);

        if (sd.getShippingBoxWeight() == null)
        {
            throw new KKException("ShippingBoxWeight is null in ShippingMgr");
        }

        if (sd.getShippingMaxWeight() == null)
        {
            throw new KKException("ShippingMaxWeight is null in ShippingMgr");
        }
            ...
            for (int j = 0; j < op.getQuantity(); j++)
            {
                ...
                if (newWeight.compareTo(maxWeight) > 0)
                {
                    ...
                    for (Iterator<BigDecimal> iter = orderWeightList.iterator(); iter.hasNext();)
                    {
                        ...
                        if (packageWeight.add(op.getProduct().getWeight())
                                .compareTo(maxWeight) <= 0)
                        {
        ...
        for (Iterator<String> iter = sd.getShippingModuleList().iterator(); iter.hasNext();)
        {
            ...
            if (modName != null)
            {
                ...
                {
                    log.error("Could not instantiate the Shipping Module " + modName + " : "
    ...
    protected void checkOrder(Order order) throws KKException
    {
        ...
        {
            throw new KKException(
                    "The order id = " + order.getId() + "does not have a valid delivery country");
        }
    ...
    public ResourceBundle getResourceBundle(Locale locale) throws KKException
    {
        synchronized (mutex)
        {
            ...
            if (rb == null)
            {
                rb = ResourceBundle.getBundle(BaseModule.basePackage + ".shipping.Shipping",
                        locale);
     ...
     */
    ShippingInterface getShippingModuleForName(String moduleName)
