...
public class OrderTotalMgr extends BaseMgr implements OrderTotalMgrIf
{
    ...
    public void refreshConfigs() throws Exception
    {
        synchronized (mutex)
        {
            ...
            {
                if (!updateStaticVariablesNow(sd.getLastUpdatedMS()))
                {
                    return;
                }
            }
            ...
            for (Iterator<String> iter = otModuleListStr1.iterator(); iter.hasNext();)
            {
                ...
                if (moduleName != null)
                {
                    try
                    {
                        OrderTotalInterface orderTotalModule = getOrderTotalModuleForName(
                                moduleName);
                        orderTotalModule.setStaticVariables();
                        if (log.isDebugEnabled())
                        {
                            log.debug("Created Order Total Module : " + moduleName + " "
                                    + orderTotalModule.getModuleClassName() + " Enabled = "
                                    + orderTotalModule.isAvailable());
                        }
                    } catch (Throwable e)
                    {
            ...
            sd.setLastUpdatedMS(System.currentTimeMillis());
            ...
            if (log.isInfoEnabled())
            {
                if (log.isDebugEnabled())
                {
                    log.debug(JavaUtils.dumpAllStackTraces(".*JavaUtils.dumpAllStackTraces.*",
                            KKConstants.DEBUG_STACK_TRACE_EXCLUSIONS));
                }
                String staticD = "Configuration data for the OrderTotalMgr on " + getStoreId();
                staticD += "\n\t\t\t Price With Tax?    = " + sd.isDispPriceWithTax();
                staticD += "\n\t\t\t Order Totals List  = " + sd.getOrderTotalModuleListCopy().size()
                        + " modules";
                staticD += "\n\t\t\t LastUpdated        = " + sd.getLastUpdatedMS();
                log.info(staticD);
            }
    ...
    public Order getOrderTotals(OrderIf order, int languageId) throws Exception
    {
        ...
        if (lang == null)
        {
            throw new KKException(
                    "A language object could not be found for a language Id of " + languageId);
        }
        ...
        if (log.isDebugEnabled())
        {
            log.debug("checkoutOrder: " + order.getLifecycleId() + " OrderTotals set to null");
        }
        ...
        if (log.isDebugEnabled())
        {
            String str = "Sorted Order Total Modules:";
            int maxLength = -1;
            for (Iterator<OrderTotalInterface> iter = otModuleList.iterator(); iter.hasNext();)
            {
                OrderTotalInterface orderTotalModule = iter.next();
                if (orderTotalModule.getCode() != null
                        && orderTotalModule.getCode().length() > maxLength)
                {
                    maxLength = orderTotalModule.getCode().length();
                }
            }
            for (Iterator<OrderTotalInterface> iter = otModuleList.iterator(); iter.hasNext();)
            {
                OrderTotalInterface orderTotalModule = iter.next();
                str += "\n\t\t " + Utils.padLeft(orderTotalModule.getSortOrder(), 4) + " "
                        + Utils.padRight(orderTotalModule.getCode(), maxLength + 2) + " Enabled? "
                        + orderTotalModule.isAvailable() + "  "
                        + orderTotalModule.getModuleClassName();
            }
            log.debug(str);
        }
        ...
        for (Iterator<OrderTotalInterface> iter = otModuleList.iterator(); iter.hasNext();)
        {
            ...
            try
            {
                ...
                if (ot != null)
                {
                    if (ot.getOrderTotals() != null && ot.getOrderTotals().length > 1)
                    {
                        for (int i = 0; i < ot.getOrderTotals().length; i++)
                        {
                            ...
                            if (log.isDebugEnabled())
                            {
                                log.debug("Add OT: " + lot.getTitle() + " " + lot.getText() + " : "
                                        + orderTotalModule.getModuleClassName());
                            }
                        }
                    } else
                    {
                        ...
                        if (log.isDebugEnabled())
                        {
                            log.debug("Add OT: " + ot.getTitle() + " " + ot.getText() + " : "
                                    + orderTotalModule.getModuleClassName());
                        }
                    }
                } else
                {
                    if (log.isDebugEnabled())
                    {
                        log.debug("Did not add OT for " + orderTotalModule.getCode() + " : "
                                + orderTotalModule.getModuleClassName());
                    }
                }
            } catch (KKException e)
        ...
        if (log.isDebugEnabled())
        {
            String msg = "Order Totals to be set on order " + order.getLifecycleId() + " :";
            int maxClassName = -1;
            int maxTitle = -1;
            for (int j = 0; j < retArray.length; j++)
            {
                OrderTotal ot = retArray[j];
                if (ot.getClassName() != null && ot.getClassName().length() > maxClassName)
                {
            ...
            for (int j = 0; j < retArray.length; j++)
            {
                ...
                msg += "\n\t\t " + Utils.padRight(ot.getClassName(), maxClassName + 2)
                        + Utils.padRight(ot.getTitle(), maxTitle + 2) + ot.getText();
            }
            log.debug(msg);
        }
        ...
        if (log.isDebugEnabled())
        {
            String msg = "Returning order: " + order.getLifecycleId() + " with ";
            if (order.getOrderTotals() == null)
            {
                msg += " NULL OrderTotals";
            } else
            {
                msg += " " + order.getOrderTotals().length + " OrderTotals";
            }
            log.debug(msg);
        }
