...

{
     ...
     * @throws KKException
     *             unexpected exception in KonaKart
     * @throws KKAppException
     *             unexpected exception in the application engine
     */
    ...
    protected String handleException(HttpServletRequest request, Exception e)
    {
        ...
        try
        {
            if (appEng != null && appEng
                    .getConfigAsBoolean(KKConstants.CONF_KEY_REPORT_STOREFRONT_EXCEPTIONS, false))
            {
    ...
    {
        return loggedIn(request, response, kkAppEng, forwardAfterLogin,
                /* checkXSRF */true, /* xsrfToken */
                null);
    }
     ...
     */
    protected int loggedIn(HttpServletRequest request, HttpServletResponse response,
            KKAppEng kkAppEng, String forwardAfterLogin, String xsrfToken)
     ...
     *            the HttpServletResponse response
     * @param checkXSRF
     *            whether or not to check the XSRF token
     * @param xsrfToken
     *            XSRF token
     * @return Returns the CustomerId if logged in. Otherwise a negative number.
     * @throws KKException
     ...
     *             unexpected exception in KonaKart
     */
    protected String checkSSL(KKAppEng eng, HttpServletRequest request, int custId,
                ...
                } catch (NumberFormatException e)
                {
                    log.warn(
                            "The product page size value stored in the cookie for customer with guest id "
                                    + guestCustomerIdStr + " is not a numeric value: "
                                    + prodPageSizeStr);
                }
            }
     ...
     * to update the customer's PRODUCTS_TIME_VIEWED tag value from the value of the guest
     * customer's tag. When logging out we need to make the same decision in the opposite direction.
     * We only do the updates if the tag value of the "oldTag" is more recent than the tag value of
     * the "newTag".
     * 
     * @param kkAppEng
     *            the KonaKart Application engine
     * @param oldTag
            ...
            CustomerTagIf newTag) throws KKAppException, KKException
    {
                ...
                && oldTag.getValue().length() > 0)
        {
                    ...
                    || newTag.getDateAdded().before(oldTag.getDateAdded()))
            {
                 ...
                 * value of old tag
                 */
                kkAppEng.getCustomerTagMgr().insertCustomerTag(TAG_PRODUCTS_TIME_VIEWED,
                        oldTag.getValue());
            }
        }
    }
     ...
     *             unexpected exception in the application engine
     */
    protected LoginResultIf login(KKAppEng kkAppEng, HttpServletRequest request,
            String loginUsername, String password) throws KKException, KKAppException
    {
        if (!kkAppEng.isPortlet())
        {
            // Change the session
    ...
    protected void changeSession(HttpServletRequest request)
    {
        ...
        {
            log.warn(
                    "Exception occurred while trying to change session for security reasons typically after a login",
                    e);
        }
    }
     ...
     *             an unexpected UnsupportedEncodingException exception
     */
