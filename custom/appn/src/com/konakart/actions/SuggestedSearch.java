...
public class SuggestedSearch extends BaseAction
{
    ...
    public String execute()
    {
        ...
        try
        {
            ...
            {
                /*
                 * Create a return array avoiding duplicate products which may exist is a product
                 * has been indexed multiple times because it exists in multiple categories
                 */
                ArrayList<SearchResult> srList = new ArrayList<SearchResult>();
                HashMap<Integer, String> srMap = new HashMap<Integer, String>();
                for (int i = 0; i < ssArray.length; i++)
                {
                    SuggestedSearchItemIf ss = ssArray[i];
                    if (ss.getProductId() > -1)
                    {
                        if (srMap.get(ss.getProductId()) == null)
                        {
                            srMap.put(ss.getProductId(), "");
                        } else
                        {
                            continue;
                        }
                    ...
                    srList.add(new SearchResult(
                            ...
                            ss.getProductId() + "," + ss.getManufacturerId() + "," + ss.getCategoryId()));
                }
                if (srList.size() > 0)
                {
                    srArray = srList.toArray(new SearchResult[srList.size()]);
                }
            }
