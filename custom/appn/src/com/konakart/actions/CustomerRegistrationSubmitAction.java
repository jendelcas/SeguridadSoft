...
public class CustomerRegistrationSubmitAction extends BaseAction
{
    ...
    public String execute()
    {
        ...
        try
        {
             ...
             * http ://localhost:8780/konakart/EnableCustomer.action?key=70168e16-eb49-45c4-b47b-
             * e2a2f8c0e6f5
             */
            boolean doubleOptIn = kkAppEng.getConfigAsBoolean(
                    KKConstants.CONF_KEY_ENABLE_REGISTRATION_DOUBLE_OPT_IN,
            ...
            if (doubleOptIn)
            {
                ...
                if (url == null || url.length() == 0)
                {
                    throw new KKException(
                            "The KK_BASE_URL has not been defined. Cannot send double opt-in email");
                }
                if (!url.substring(url.length() - 1).equals("/"))
                {
                    url += "/";
                }
                url += ("EnableCustomer.action?key=" + secretKey + "&m=" + getEmailAddr() + "&i=" + customerId);

                EmailOptionsIf options = new EmailOptions();
                options.setCountryCode(kkAppEng.getLocale().substring(0, 2));
                options.setTemplateName("RegistrationDoubleOptIn");
                options.setCustomAttrs(new NameValue[]
                { new NameValue("linkURL", url) });
                kkAppEng.getEng().sendTemplateEmailToCustomer1(customerId, "", options);
                message = kkAppEng.getMsg("double.opt.in.registration.message", getEmailAddr());
                return "DoubleOptIn";
            }
            ...
            // Set customer tags
            setCustomerTags(kkAppEng, birthDate, getCountryId(), getGender());

            // Set customer reward points
            setCustomerRewardPoints(kkAppEng);
     ...
     */
    protected void setCustomerRewardPoints(KKAppEng kkAppEng) throws KKAppException
    {
     ...
     */
    protected void setCustomerTags(KKAppEng kkAppEng, Date birthDate, int countryId, String gender)
            throws KKAppException, KKException
    {
        ...
        {
            kkAppEng.getCustomerTagMgr().insertCustomerTag(TAG_COUNTRY_CODE, country.getIsoCode3());
        }
        ...
        if (gender != null && gender.equalsIgnoreCase(KKConstants.GENDER_MALE))
        {
