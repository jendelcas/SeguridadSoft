...
import com.konakart.al.JsonException;
...
public class BaseGatewayAction extends BaseAction
{
    ...
    protected boolean enableJsonExceptionHandling = false;
    ...
    /**
     * Send the payment data. Either use a GET or a POST depending on the value in the
     * PaymentDetails parameter.
     * 
     * @param pd payment details
     * @param parmList parameter list
     * @return the response from the gateway as a string
     * @throws IOException if we get an IOException communication with the gateway
     */
    public String sendPaymentData(PaymentDetailsIf pd, List<NameValueIf> parmList) throws IOException
    {
        if (pd.getPostOrGet().equalsIgnoreCase("POST"))
        {
            return postData(pd, parmList);
        } else if (pd.getPostOrGet().equalsIgnoreCase("GET"))
        {
            return getData(pd, parmList);
        }
        
        log.warn("Unrecognised Post or Get Value: '" + pd.getPostOrGet() + "' - use a POST:");
        return postData(pd, parmList);
    }
    ...
    public String getData(PaymentDetailsIf pd, List<NameValueIf> ccParmList) throws IOException
    {
        ...
        if (!urlStr.endsWith("/") && !ccParmList.isEmpty())
        {
            urlStr += "?";
        }
        ...
        customizeConnection(conn, pd, ccParmList);
            ...
            String konakartResultDescription, int konakartResultId) throws KKException
    {
        ...
        saveIPNrecord(kkAppEng, orderId, moduleCode, gatewayFullResponse, gatewayResult,
                gatewayTransactionId, konakartResultDescription, konakartResultId, txType,
                txAmount);
    }
            ...
            BigDecimal txAmount) throws KKException
    {
        ...
        {
            log.warn("Exception occurred trying to save IPN History record: " + e.getMessage());
            throw e;
        }
    }
    ...
    {
        if (enableJsonExceptionHandling)
        {
            ...
            try
            {
                if (appEng.getConfigAsBoolean(KKConstants.CONF_KEY_REPORT_STOREFRONT_EXCEPTIONS,
                        false))
                {
            ...
            if (e != null)
            {
                ...
                {
                    KKException ex = (KKException) e;
                    exception = new JsonException(ex.getCode(), ex.getMessage());
                } else
                {
                    exception = new JsonException(0, e.getMessage());
                }

            } else
            {
                exception = new JsonException(0, "Unknown Error");
            }
    ...
    /**
     * @return the enableJsonExceptionHandling
     */
    public boolean isEnableJsonExceptionHandling()
    {
        return enableJsonExceptionHandling;
    }

    /**
     * @param enableJsonExceptionHandling
     *            the enableJsonExceptionHandling to set
     */
    public void setEnableJsonExceptionHandling(boolean enableJsonExceptionHandling)
    {
        this.enableJsonExceptionHandling = enableJsonExceptionHandling;
    }
