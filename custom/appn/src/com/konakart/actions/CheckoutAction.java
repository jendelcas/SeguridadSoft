...
public class CheckoutAction extends BaseAction
{
    ...
    public String execute()
    {
        ...
        try
        {
             ...
             */
            synchronized (kkAppEng.checkoutMutex)
            {
                ...
                String stockAllowCheckout = kkAppEng
                        .getConfig(ConfigConstants.STORE_FRONT_STOCK_ALLOW_CHECKOUT);
                ...
                if (stockAllowCheckout != null && stockAllowCheckout.equalsIgnoreCase("false"))
                {
                    if (reserveStock != null && reserveStock.equalsIgnoreCase("true"))
                    {
                        ...
                        BasketIf[] items = kkAppEng.getBasketMgr()
                                .reserveStock(cust.getBasketItems());
                        for (int i = 0; i < items.length; i++)
                        {
                            ...
                            if (basket.getReservationId() <= 0)
                            {
                                if (basket.getQuantityAvailable() > 0)
                                {
                                    ...
                                    addActionError(kkAppEng.getMsg(
                                            "edit.cart.body.limitedstock.error",
                                            basket.getProduct().getName(),
                                            Integer.toString(basket.getQuantityAvailable())));
                                } else if (basket.getQuantityReserved() > 0)
                                {
                                    // Inform that some stock is currently reserved and could become
                                    // available later
                                    addActionError(kkAppEng.getMsg(
                                            "edit.cart.body.reservedstock.error",
                                            basket.getProduct().getName(),
                                            Integer.toString(basket.getQuantityReserved())));
                                } else
                                {
                                    addActionError(
                                            kkAppEng.getMsg("edit.cart.body.outofstock.error",
                                                    basket.getProduct().getName(),
                            ...
                            if (basket.getReservationStartDate() != null
                                    && basket.getReservationExpiryDate() != null)
                            {
                                long endMillis = basket.getReservationExpiryDate()
                                        .getTimeInMillis();
                                long startMillis = basket.getReservationStartDate()
                                        .getTimeInMillis();
                    ...
                    } else
                    {
                        // If required, we check to see whether the products are in stock
                        BasketIf[] items = kkAppEng.getBasketMgr()
                                .updateBasketWithStockInfo(cust.getBasketItems());
                        for (int i = 0; i < items.length; i++)
                        {
                            ...
                            if (basket.getQuantity() > basket.getQuantityInStock())
                            {
                                ...
                                {
                                    addActionError(
                                            kkAppEng.getMsg("edit.cart.body.limitedstock.error",
                                                    basket.getProduct().getName(),
                                                    Integer.toString(basket.getQuantityInStock())));
                                } else
                                {
                ...
                {
                    /*
                     * New order is populated with coupon code, gift certificate and reward points
                     * if these were entered in the edit cart screen
                     */
                ...
                if (kkAppEng.isMultiVendor() && order.getVendorOrders() != null)
                {
                    ...
                    for (int i = 0; i < order.getVendorOrders().length; i++)
                    {
                        ...
                        ShippingQuoteIf[] vQuotes = kkAppEng.getOrderMgr()
                                .getVendorShippingQuoteMap().get(vOrder.getStoreId());
                        String desc = null;
                        ...
                        kkAppEng.getOrderMgr().addShippingQuoteToVendorOrder(vShipping[i], vOrder,
                                desc);
                        // Add values to quote for main order
                        if (vOrder.getShippingQuote() != null)
                        {
                            sumOfVendorQuotes.setTax(sumOfVendorQuotes.getTax()
                                    .add(vOrder.getShippingQuote().getTax()));
                            sumOfVendorQuotes.setTotalExTax(sumOfVendorQuotes.getTotalExTax()
                                    .add(vOrder.getShippingQuote().getTotalExTax()));
                ...
                checkoutMsg = callouts.validateOrderTotals(kkAppEng,
                ...
                if (checkoutMsg == null)
                {
                    checkoutMsg = callouts.validateOrderForCustomer(kkAppEng,
                            kkAppEng.getOrderMgr().getCheckoutOrder());
                }
    ...
    /**
     * @param stockExpirySecs
     *            the stockExpirySecs to set
     */
