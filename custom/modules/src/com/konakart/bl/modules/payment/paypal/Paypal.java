...
public class Paypal extends BasePaymentModule implements PaymentInterface
{
     ...
     */
    public final static String MODULE_PAYMENT_PAYPAL_TEST_MODE = "MODULE_PAYMENT_PAYPAL_TEST_MODE";
    ...
    public PaymentDetails getPaymentDetails(Order order, PaymentInfo info) throws Exception
    {
        ...
        parmList.add(new NameValue("item_name",
                "Order #" + order.getId() + " from " + info.getStoreName()));
        ...
        /*
         * Create and save an sso token and save the order id, the order total and the customer id
         */
        SSOTokenIf ssoToken = new SSOToken();
        ssoToken.setCustom1(String.valueOf(order.getId()));
        ssoToken.setCustom2(total.toString());
        ssoToken.setCustomerId(order.getCustomerId());
        ...
        parmList.add(new NameValue("notify_url",
                sd.getPayPalCallbackUrl().replaceFirst(hostPortSubstitute, info.getHostAndPort())));

        sd.setPayPalReturnUrl(
                sd.getPayPalReturnUrl().replaceFirst(hostPortSubstitute, info.getHostAndPort()));
        sd.setPayPalCancelUrl(
                sd.getPayPalCancelUrl().replaceFirst(hostPortSubstitute, info.getHostAndPort()));
    ...
    /**
     * Used by the action class to get a session id. The session id returned is a session id for the
     * customer id found in the SSO token.
     * 
     * @param sessionId the session Id
     * @param parameters an array of name value parameters
     * @return Returns information in a PaymentDetails object
            ...
            throws Exception
    {
        ...
        String secretKey = parameters[0].getValue();
        SSOTokenIf token = getEng().getSSOToken(secretKey, /* deleteToken */false);
        if (token == null || token.getCustomerId() == 0)
        {
            return null;
        }

        String session = getSecMgr().login(token.getCustomerId());
        if (session == null)
        {
            throw new KKException(
                    "Unable to get a session for customer with id = " + token.getCustomerId());
        }
        PaymentDetails pDetails = new PaymentDetails();
        pDetails.setCustom1(session);
        return pDetails;
    }
