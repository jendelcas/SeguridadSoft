...
public class TaxCloud extends BaseOrderTotalModule implements OrderTotalInterface
{
    ...
    public void setStaticVariables() throws KKException
    {
        ...
        {
            throw new KKException("The store address for store " + getStoreId()
        ...
        if (log.isInfoEnabled())
        {
            ...
            staticD += "\n\t\t Verified Address1  = " + staticData.getOriginAddr().getAddress1();
            staticD += "\n\t\t Verified Address2  = " + staticData.getOriginAddr().getAddress2();
            staticD += "\n\t\t Verified City      = " + staticData.getOriginAddr().getCity();
            staticD += "\n\t\t Verified State     = " + staticData.getOriginAddr().getState();
            staticD += "\n\t\t Verified Zip5      = " + staticData.getOriginAddr().getZip5();
            staticD += "\n\t\t Verified Zip4      = " + staticData.getOriginAddr().getZip4();
            staticD += "\n\t\t TaxCloud Login Id  = " + staticData.getLoginId();
            staticD += "\n\t\t USPSID             = " + staticData.getUspsId();
            log.info(staticD);
        }
            ...
            throws TorqueException, DataSetException, Exception
    {
         ...
         * one.
         */
        int arrayLength = (isShipping) ? order.getOrderProducts().length + 1
                : order.getOrderProducts().length;
        ...
        for (int i = 0; i < order.getOrderProducts().length; i++)
        {
            ...
            BigDecimal singleProdPrice = (orderProduct.getFinalPriceExTax()).divide(
                    new BigDecimal(orderProduct.getQuantity()), scale, BigDecimal.ROUND_HALF_UP);
        ...
        order.setDeliveryPostcode(
                destinationVerified.getZip5() + "-" + destinationVerified.getZip4());
    ...
    private void setZipCodes(Address address)
    {
        // In the case where the zipcode specified is of the form NNNNN-NNNN we take the first 5
        // numbers and set these in the zip5 attribute and the last 4 numbers in the zip4 attribute.
        ...
        if (address.getZip5().length() != 5)
        {
            final String regex = "^[0-9]{5}-[0-9]{4}$";
            Pattern pattern = Pattern.compile(regex);
            if (pattern.matcher(address.getZip5()).matches())
            {
                address.setZip4(address.getZip5().substring(6));
                address.setZip5(address.getZip5().substring(0, 5));
                if (log.isInfoEnabled())
                {
                    log.info("Zip code for " + address.getAddress1() + " split into Zip5 = "
                            + address.getZip5() + " and Zip4 = " + address.getZip4());
                }
            }
        }
    }
    ...
    public void commitOrder(OrderIf order) throws Exception
    {
        try
        {
            ...
            if (sd == null)
            {
                log.warn("TaxCloud StaticData is null - try to set it now");
                setStaticVariables();                
                sd = staticDataHM.get(getStoreId()); 
            }
