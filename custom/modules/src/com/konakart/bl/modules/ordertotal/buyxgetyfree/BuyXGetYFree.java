            ...
            for (int i = 0; i < promArray.length; i++)
            {
                ...
                for (int j = 0; j < promotion.getApplicableProducts().length; j++)
                {
                    ...
                    if (op != null && op.getQuantity() >= buyPlusFree)
                    {
                        ...
                        // Apply the discount
                        BigDecimal discountPerBuyFreeGroup = currentPrice.multiply(
                                new BigDecimal(Math.floor(op.getQuantity() / buyPlusFree)));
                        BigDecimal discount = discountPerBuyFreeGroup
                                .multiply(new BigDecimal(freeProdQuantity));
