...
<%String addToCartOrWishList=null; %>
	        	...
	        	<%}%>
						...
						<div id="product-price">
							...
							<%java.math.BigDecimal actualPrice = null;%>
					        ...
					        <%} else {%>
					            <%if (prod.getSpecialPriceExTax() != null) {%>
					                ...
					                <%actualPrice = prod.getSpecialPriceExTax();%>
					        	<%} else {%>
					                <span class="product-price-current"><%=kkEng.formatPrice(prod.getPriceExTax())%></span>
					                <%actualPrice = prod.getPriceExTax();%>
					            <%}%>
							<%}%>
							<%if (prod.getNetWeightOrVolumeUOM() != null && prod.getNetWeightOrVolumeUOM().length() > 0 
								&& prod.getNetWeightOrVolume() != null && prod.getNetWeightOrVolume().compareTo(BigDecimal.ZERO) != 0){%>
								<%int scale = Integer.parseInt(kkEng.getDefaultCurrency().getDecimalPlaces());%>
								<%BigDecimal pricePerUOM = actualPrice.divide(prod.getNetWeightOrVolume(),scale,BigDecimal.ROUND_HALF_UP);%>
							    <span class="product-price-uom">(<%=kkEng.formatPrice(pricePerUOM)%>/<%=prod.getNetWeightOrVolumeUOM()%>)</span>
							<%}%>
						</div>
				        ...
				        <%}%>
   						...
   						<div id="notify-me-container">
							...
							<%if (kkEng.isMultiVendor() && kkEng.isMultiVendorStoreLinks() && prod.getStoreName() != null) { %>	
								<br>			
								<a href="StoreDetails.action?id=<%=prod.getStoreId()%>">
									<kk:msg  key="product.details.body.sold.by"/> <b><%=prod.getStoreName()%></b>
								</a>
     					    <% } %> 			
     					</div>  	
					</form>