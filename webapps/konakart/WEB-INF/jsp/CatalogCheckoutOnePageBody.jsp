...
function getOrderProductRow(result, op) {	
 	var row = '<tr><td>';
 	row += '<img class="product-image" src="'+op.prodImgSrc+'" border="0" alt="'+op.name+'" title="'+op.name+'"/>';
 	row += '</td><td>';
					    					...
					    					<tbody id="ot-table_<%=vOrder.getStoreId()%>">
				   								<%if (vOrder.getOrderProducts() != null && vOrder.getOrderProducts().length > 0){ %>
													<% for (int j = 0; j < vOrder.getOrderProducts().length; j++){ %>
														...
														<tr>
															<td>
																<img class="product-image" src="<%=kkEng.getProdImage(op.getProduct(), op.getOpts(), com.konakart.al.KKAppEng.IMAGE_TINY)%>" border="0" alt="<%=op.getProduct().getName()%>" title="<%=op.getProduct().getName()%>"/>
															</td>
															<td>
													...
													<% for (int j = vOrder.getOrderTotals().length-1; j > -1; j--){ %>
														...
														<%if (ot.getClassName().equals("ot_shipping")){%>
															<tr>
																<td></td>
																<td>
																	<label style='width:100%;' id="shipping_name_<%=vOrder.getStoreId()%>"><%=ot.getTitle()%></label><br/>
