...
<% com.konakart.appif.StoreIf store = customerMgr.getCurrentStore();%>	
				...
				<h1 id="page-title"><kk:msg  key="write.review.body.title" arg0="<%=store.getStoreName()%>"/></h1>
	    		<div class="content-area rounded-corners">
		    		<div>
			    		<form action="WriteVendorReviewSubmit.action" id="form1" method="post">
							<input type="hidden" value="<%=kkEng.getXsrfToken()%>" name="xsrf_token"/>
							<input type="hidden" value="<%=vendor.getId()%>" name="vendorId"/>
							<input type="hidden" value="<%=store.getStoreId()%>" name="vendorStoreId"/>
							...
							<div class="form-buttons-write-review">
								...
								<a href='<%="StoreDetails.action?id="+store.getStoreId()%>' class="button small-rounded-corners"><span><kk:msg  key="common.back"/></span></a>							                        				
							</div>
						</form>
			    	</div>
	    		</div>
