...
public class GetCustomerExamples extends BaseApiExample
{
    ...
    public static void main(String[] args)
    {
        try
        {
            ...
            /**
             * In this third run we instantiate the admin JSON client engine. This will send
             * our custom() call over JSON to a running KonaKart system (identified by the URL in
             * konakartadmin.properties). The engine that is instantiated by that remote
             * system running the JSON services will instantiate the engine identified in the
             * konakartadmin.properties file by the konakart.app.json.engine.classname property
             */
            test.testUsingEngine("com.konakartadmin.json.KKJSONAdminEng", null);

            /**
             * In this fourth run we instantiate the admin JSON Services client engine again but this
             * time using the properties array to specify the URL of the JSON service. Again this
             * will send our custom() call over SOAP to a running KonaKart system (identified by the
             * URL in konakartadmin.properties). The engine that is instantiated by that
             * remote system running the web services will instantiate the engine identified in the
             * konakartadmin.properties file by the konakart.app.json.engine.classname property
             */
            test.testUsingEngine("com.konakartadmin.json.KKJSONAdminEng", test.getPropertiesFromFile(
                    KKConstants.KONAKARTADMIN_PROPERTIES_FILE, null));
            ...
            throws KKAdminException, KKException, ConfigurationException
    {
        ...
        PropertiesConfiguration conf = PropertiesUtils.getConfiguration(configFileURL);
