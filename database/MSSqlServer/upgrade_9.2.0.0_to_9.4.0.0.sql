...
INSERT INTO kk_config (kk_config_key, kk_config_value, date_added) VALUES ('HISTORY', '9.4.0.0 U', getdate());
UPDATE kk_config SET kk_config_value='9.4.0.0 SQLServer', date_added=getdate() WHERE kk_config_key='VERSION';
...
INSERT INTO configuration (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added, return_by_api, store_id) SELECT 'Output Queue Prefix','ERP_OUTPUT_QUEUE_PREFIX','KonaKart.','Output queue name for ERP integration is this prefix + storeId + .Q',32,110,getdate(),'1', store_id FROM configuration WHERE configuration_key = 'INSTALLATION_HOME';

-- New configuration variable to allow setting of STARTTLS for emails
INSERT INTO configuration (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added, return_by_api, store_id) SELECT 'SMTP StartTLS','SMTP_STARTTLS','false','Whether or not to enable STARTTLS', '12', '3',getdate(),'1', store_id FROM configuration WHERE configuration_key = 'INSTALLATION_HOME';

-- New configuration variable to be used for custom purposes as a System Id
INSERT INTO configuration (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added, return_by_api, store_id) SELECT 'System ID','SYSTEM_ID','','Can be used for any custom purpose to identify the system',32,120,getdate(),'1', store_id FROM configuration WHERE configuration_key = 'INSTALLATION_HOME';
...
INSERT INTO configuration (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added, return_by_api, store_id) SELECT 'Run Initial Product Search', 'PROD_PANEL_RUN_INITIAL_SEARCH', 'true','Set to true to always run the initial Product Search',21, 13,'choice(''true''=''true'',''false''=''false'')', getdate(),'1', store_id FROM configuration WHERE configuration_key = 'INSTALLATION_HOME';
