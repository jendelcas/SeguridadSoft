...
INSERT INTO kk_config (kk_config_key, kk_config_value, date_added) VALUES ('HISTORY', '8.9.0.0 U', getdate());
UPDATE kk_config SET kk_config_value='8.9.0.0 SQLServer', date_added=getdate() WHERE kk_config_key='VERSION';
...
INSERT INTO configuration (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added, return_by_api, store_id) SELECT 'Enable Newsletter Double Opt-In','ENABLE_NEWSLETTER_DOUBLE_OPT_IN','false','When true, a customer receives an eMail with a link to click on to confirm newsletter subscription',1, 65, 'choice(''true''=''true'',''false''=''false'')', getdate(), '1', store_id FROM configuration WHERE configuration_key = 'INSTALLATION_HOME';
...
-- Drop the index on store_id before dropping that column in MS SQL Server
DROP INDEX i_stproducts_to_categories ON products_to_categories
...
ALTER TABLE customers ALTER COLUMN customers_password NVARCHAR(96);
ALTER TABLE kk_cust_pwd_hist ALTER COLUMN password NVARCHAR(96);
...
INSERT INTO kk_panel (code, description, date_added) VALUES ('kk_panel_integrations','Configuration for Integrations', getdate());
...
INSERT INTO kk_customer_tag (name, description, validation, tag_type, max_ints, set_by_api, date_added, store_id) SELECT 'PRODUCTS_TIME_VIEWED', 'Recently viewed product id', '((:[0-9]+ [0-9]+)+:|)', 7, 10, 1, getdate(), store_id FROM (SELECT store_id FROM kk_customer_tag GROUP BY store_id) STORES;
DELETE FROM kk_customer_tag WHERE name ='CATEGORIES_TIME_VIEWED';
INSERT INTO kk_customer_tag (name, description, validation, tag_type, max_ints, set_by_api, date_added, store_id) SELECT 'CATEGORIES_TIME_VIEWED', 'Recently viewed category id', '((:[0-9]+ [0-9]+)+:|)', 7, 10, 1, getdate(), store_id FROM (SELECT store_id FROM kk_customer_tag GROUP BY store_id) STORES;
DELETE FROM kk_customer_tag WHERE name ='MANUFACTURERS_TIME_VIEWED';
INSERT INTO kk_customer_tag (name, description, validation, tag_type, max_ints, set_by_api, date_added, store_id) SELECT 'MANUFACTURERS_TIME_VIEWED', 'Recently viewed manufacturer id', '((:[0-9]+ [0-9]+)+:|)', 7, 10, 1, getdate(), store_id FROM (SELECT store_id FROM kk_customer_tag GROUP BY store_id) STORES;
DELETE FROM kk_customer_tag WHERE name ='PRODUCTS_TIME_IN_CART';
INSERT INTO kk_customer_tag (name, description, validation, tag_type, max_ints, set_by_api, date_added, store_id) SELECT 'PRODUCTS_TIME_IN_CART', 'Id of a product in the cart', '((:[0-9]+ [0-9]+)+:|)', 7, 10, 1, getdate(), store_id FROM (SELECT store_id FROM kk_customer_tag GROUP BY store_id) STORES;
DELETE FROM kk_customer_tag WHERE name ='PRODUCTS_TIME_IN_WISHLIST';
INSERT INTO kk_customer_tag (name, description, validation, tag_type, max_ints, set_by_api, date_added, store_id) SELECT 'PRODUCTS_TIME_IN_WISHLIST', 'Id of a product in the Wish List', '((:[0-9]+ [0-9]+)+:|)', 7, 10, 1, getdate(), store_id FROM (SELECT store_id FROM kk_customer_tag GROUP BY store_id) STORES;
exit;
